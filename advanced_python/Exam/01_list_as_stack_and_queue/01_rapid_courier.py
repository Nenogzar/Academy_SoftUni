...
while packages and couriers:
    ...
    weight_difference = courier_capacity - package_weight

    if weight_difference < 0:
        packages.append(package_weight - courier_capacity)

    elif weight_difference - package_weight > 0:
        couriers.append(weight_difference - package_weight)
...
if not packages and not couriers:
    ...
    print(
        f"TCouriers are still on duty: {', '.join(str(x) for x in couriers)} but there are no more packages to deliver.")
