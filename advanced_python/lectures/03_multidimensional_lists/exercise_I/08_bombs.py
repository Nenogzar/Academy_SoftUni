...
bombs = [[int(i) for i in bomb.split(',')] for bomb in input().split()]

for bomb in bombs:
    x, y = bomb
    power_of_bomb = matrix[x][y]
    if power_of_bomb > 0:
        matrix[x][y] = 0
...
output_message = f'Alive cells: {len(alive_cells)}\n'
output_message += f'Sum: {sum(alive_cells)}\n'
...
def start_bombing():
    for bomb in bombs:
        ...
        if power_of_bomb > 0:
            matrix[x][y] = 0
            collateral_damage = check_range_of_bomb(x, y)
            detonate_bomb_and_damage_cells(collateral_damage, power_of_bomb)

# Check range of bomb
def check_range_of_bomb(x, y):
    cells_for_destroy = []
    for row in range(x - 1, x + 2):
        for col in range(y - 1, y + 2):
            if 0 <= row < rows and 0 <= col < len(matrix[row]):
...
# Detonate bomb and damage cells
def detonate_bomb_and_damage_cells(cells, explosion):
    for target in cells:
        cell_value = matrix[target[0]][target[1]]
        if cell_value > 0:
            matrix[target[0]][target[1]] -= explosion
