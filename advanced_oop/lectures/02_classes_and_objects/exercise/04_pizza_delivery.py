...
# *******  TASK CONDITION  ******* #
"""
 https://judge.softuni.org/Contests/Compete/Index/1937#3
...
-	add_extra(ingredient: str, quantity: int, price_per_quantity: float):
    ...
    increase the ingredient quantity with the given one and
    update the pizza price by adding the ingredient price for the given quantity
...
-	remove_ingredient(ingredient: str, quantity: int, price_per_quantity: float):
    ...
    o	If we have the ingredient, but we try to remove more than we have available,
        we should return the following message
            "Please check again the desired quantity of {ingredient}!"
    o	Otherwise, remove the given quantity of the ingredient and
...
class PizzaDelivery:
    ...
    def __init__(self, pizza_name:str, pizza_price:float, pizza_ingredients:dict):
    ...
    def make_order(self):
        if not self.ordered:
            ...
            return f"You've ordered pizza {self.name} prepared with {', '.join(text)} " \
                   f"and the price will be {self.price:.1f}lv."
        return f"Pizza {self.name} already prepared, and we can't make any changes!"
