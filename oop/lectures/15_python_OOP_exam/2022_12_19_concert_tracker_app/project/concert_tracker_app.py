...
class ConcertTrackerApp:
    ...
    def start_concert(self, concert_place: str, band_name: str):
        ...
        genre = concert.genre
        if genre not in self.concert_requirements:
            raise Exception(f"Unknown concert genre: {genre}")

        required_skills = self.concert_requirements[genre]
        for musician in band.members:
            musician_type = type(musician).__name__
            if musician_type not in required_skills:
                raise Exception(f"Unexpected musician type: {musician_type}")

            needed_skills = required_skills[musician_type]
            if not all(skill in musician.skills for skill in needed_skills):
