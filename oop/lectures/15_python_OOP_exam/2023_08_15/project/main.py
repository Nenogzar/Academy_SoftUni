"""
Handball is a team sport played with a ball,
    where two teams of seven players (including a goalkeeper) compete to score the most goals during the game.
        The gameplay combines rapid attacks and strategic defense.
            We are developing a support application to enhance understanding and practice of the game,
...
Our task is to implement the structure and functionality
    of all the classes (properties, methods, inheritance, abstraction, etc.)
You are free to add additional attributes (instance attributes, class attributes, methods, dunder methods, etc.)
    to simplify your code and increase readability as long as it does not change
        the project's final result in accordance with its requirements so that the program works properly.

1.	Class BaseEquipment
    In the base_equipment.py file, the class BaseEquipment should be implemented.
        It is a base class for any type of equipment, and it should not be able to be instantiated.
    ...
    Methods
        ...
        increase_price()
            •	Method increases the equipment’s price.
                Keep in mind that each type of equipment implements the method differently.
