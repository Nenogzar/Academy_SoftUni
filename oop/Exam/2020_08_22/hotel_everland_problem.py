...
https://judge.softuni.org/Contests/Practice/Index/2472

Structure (Problem 1) and Functionality (Problem 2)
Our first task is to implement the structure and
    functionality of all the classes (properties, methods, inheritance, etc.)
...
7.	Class Room
    ...
    Structure
        ...
        •	members_count: int
                – passed upon initialization
        •	children: list
                – an empty list that will contain all kids in that room (objects)
        •	expenses
                – 0 by default. If it is set to a negative number,
                    raise ValueError with the message "Expenses cannot be negative"
    Methods
        __init__(name: str, budget: float, members_count: int)
            ...
            Each element of args will be a list (with children or appliances).
                Calculate the total cost for a month (30 days)
...
8.	Class AloneOld
    ...
    Methods
        ...
        This room has only one member,
            and the budget equals the pension of the person.
...
10.	Class OldCouple
    ...
    Attributes
        Apart from the attributes of the Room class,
            the OldCouple class should have a room_cost attribute equal to 15
                and a list of appliances (a tv, a fridge, and a stove for each person)
    Methods
...
12.	Class YoungCoupleWithChildren
    ...
    Methods
        __init__(family_name: str, salary_one: float, salary_two: float, *children)
            Add the children to the children attribute.
                Each child will be an instance of the Child class.
                    This room can have a different number of members (parents + children),
                        the budget equals to the two salaries of the people.
...
13.	Class Everland
    ...
    get_monthly_consumptions()
        Calculate the expenses of each room + the room_cost and return the result in the following format:
          return "Monthly consumption: {total_consumption}$."
    ...
    Return information about the hotel.
        If there are children in the room, print them first, and then the appliances monthly cost.
