...
class BaseTeam(ABC):
    ...
    def __init__(self, name: str, country: str, advantage: int):
        ...
        self.budget = self.teams[self.__class__.__name__]["budget"]
        self.wins = 0
        self.equipment = []
    ...
    def name(self, value):
        if not self.check_name(value):
            raise ValueError("Team name cannot be empty or invalid!")
        self._name = value
    ...
    def advantage(self, value):
        if int(value) <= 0:
            raise TypeError('Advantage must be greater than zero!')
